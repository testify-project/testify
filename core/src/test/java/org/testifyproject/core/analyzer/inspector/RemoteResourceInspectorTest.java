 ...
 * @author saden
 */
public class RemoteResourceInspectorTest {

    RemoteResourceInspector sut;

    @Before
    public void init() {
        sut = new RemoteResourceInspector();
    }
    ...
    public void givenParamtersInspectShouldAddProperty() {
        ...
        RemoteResource annotation = mock(RemoteResource.class);

        sut.inspect(testDescriptor, annotatedType, annotation);

        verify(testDescriptor).addCollectionElement(TestDescriptorProperties.REMOTE_RESOURCES, annotation);
    }
