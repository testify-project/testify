 ...
 * @author saden
 */
public class HintInspectorTest {

    HintInspector sut;

    @Before
    public void init() {
        sut = new HintInspector();
    }

    @Test
    public void givenParamtersInspectShouldAddProperty() {
        TestDescriptor testDescriptor = mock(TestDescriptor.class);
        Class<?> annotatedType = Hint.class;
        Hint annotation = mock(Hint.class);

        sut.inspect(testDescriptor, annotatedType, annotation);

        verify(testDescriptor).addProperty(TestDescriptorProperties.HINT_ANNOTATION, annotation);
    }
}
