...
import org.testify.container.docker.callback.PullCallback;
import org.testify.container.docker.callback.WaitCallback;
import org.testify.failsafe.Failsafe;
import org.testify.failsafe.RetryPolicy;
import org.testify.github.dockerjava.api.DockerClient;
import org.testify.github.dockerjava.api.command.CreateContainerCmd;
import org.testify.github.dockerjava.api.command.CreateContainerResponse;
import org.testify.github.dockerjava.api.command.InspectContainerResponse;
import org.testify.github.dockerjava.api.model.ContainerNetwork;
import org.testify.github.dockerjava.api.model.NetworkSettings;
import org.testify.github.dockerjava.core.DockerClientBuilder;
import org.testify.github.dockerjava.core.DockerClientConfig;
import org.testify.github.dockerjava.core.DockerClientConfig.DockerClientConfigBuilder;
...
public class DockerContainerProvider implements ContainerProvider<RequiresContainer, DockerClientConfigBuilder> {
    ...
    private void removeContainer(String containerId) {
        ...
        Failsafe.with(retryPolicy).onRetry(t -> {
            testContext.debug("Trying to remove Docker Container {}", containerId, t);
        }).onSuccess(result -> {
            testContext.info("Docker Container '{}' Removed", containerId);
            client.removeVolumeCmd(containerId).exec();
        }).onFailure(t -> {
            testContext.error("Docker Container '{}' could not be removed", containerId, t);
        }).run(() -> {
